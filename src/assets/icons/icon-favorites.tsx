import React from 'react'

import styled from 'styled-components'

type Props = {
  color?: string
}

export const IconBookMark = (props: Props): React.JSX.Element => {
  const { color } = props

  return (
    <Svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='24px' height='24px' fill={color}>
      <path d='M 6.0097656 2 C 4.9143111 2 4.0097656 2.9025988 4.0097656 3.9980469 L 4 22 L 12 19 L 20 22 L 20 20.556641 L 20 4 C 20 2.9069372 19.093063 2 18 2 L 6.0097656 2 z M 6.0097656 4 L 18 4 L 18 19.113281 L 12 16.863281 L 6.0019531 19.113281 L 6.0097656 4 z' />
    </Svg>
  )
}

const Svg = styled.svg<{ color?: string }>`
  fill: ${(props) => props.color && props.theme.color.black};
`
